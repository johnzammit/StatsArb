...
class Execution():
    ...
    async def place_order_condition(self, spread: float, upper_bollinger_band_price: float,
                                    lower_bollinger_band_price: float) -> bool:
    ...
    async def update_limit_sell(self, short_coin, take_profit_price, cancelReplaceMode, cancelOrderId, timeInForce,
    ...
    async def take_profit_long_price(self, coin: float, timestamp) -> float:
        ...
        target_price = MarketState.spread_moving_avg() - (
                decrease_rate * timestamp) + MarketState.derivative_of_spread() % MarketState.current_price(
            coin) / 100
    ...
    def main(self, market: MarketState):
        ...
        if (self.place_order_condition(spread, upper_bollinger_band_price, lower_bollinger_band_price)):
            ...
            while (self.check_open_orders()):
                ...
                if (MarketState.current_price(long_coin) <= MarketState.hard_stop_loss(
                        long_coin) or MarketState.current_price(short_coin) >= MarketState.hard_stop_loss(short_coin)):
...
if __name__ == '__main__':
    ...
    # executor = Executor(None, None, None, None)
